...
def test_secondary_aggregation(file, tmp_path):
    ...
    assert abs((original_ckt_power.real - reduced_ckt_power.real) / original_ckt_power.real) < 0.04
...
def test_primary_aggregation(file, tmp_path):
    ...
    reducer.export_original_ckt(tmp_path / "original_ckt.dss")
    reducer.export(reduced_ckt, tmp_path / "reduced_ckt.dss")
